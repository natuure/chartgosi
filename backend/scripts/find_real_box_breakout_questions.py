...
STRONG_BREAKOUT_VOLUME_RATIO = 2.00
...
def evaluate_box_breakout_candidate(candles: list[dict[str, Any]], box_start: int, breakout_index: int) -> dict[str, Any] | None:
    ...
    support_touches = sum(1 for candle in box if candle["close"] <= box_bottom * (1 + SUPPORT_TOUCH_BAND))
    if support_touches < 1:
        return None
    ...
    breakdown["support_touches"] = 10 if support_touches >= 2 else 7
    ...
    breakdown["breakout_strength"] = 15 if MIN_BREAKOUT_RATE <= breakout_rate <= OVERHEATED_BREAKOUT_RATE else 10
    breakdown["breakout_volume"] = 15 if breakout_volume_ratio >= STRONG_BREAKOUT_VOLUME_RATIO else 12
