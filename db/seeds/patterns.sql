INSERT INTO patterns (id, slug, name, description, definition, sort_order) VALUES
  ...
  (
    ...
    '{
      ...
      "confirmation": ["마지막 수축 구간의 거래량 감소", "주봉 MA10/30/40 상승 구조", "피벗 가격 근처에서 마감", "돌파 시 거래량 회복"],
      "invalidation": ["선행 상승 추세 부족", "확인 가능한 수축이 2회 미만", "뒤쪽 수축폭이 앞쪽보다 커짐", "마지막 수축에서 거래량 증가", "주요 주봉 이동평균선 이탈"],
  ...
  (
    ...
    'flag',
    '깃발형',
    'High Tight Flag 관점으로, 주봉 4~8주 급등 이후 1~10주 안의 짧고 타이트한 조정을 만드는 패턴입니다.',
    '{
      "summary": "High Tight Flag는 주봉에서 4~8주 안의 강한 급등 뒤, 급등 고점봉 종가를 넘지 않는 1~10주 조정을 만들며 힘을 모으는 추세 지속형 패턴입니다.",
      "structure": ["주봉 4~8주 안에 저점 종가 대비 고점봉 종가가 80~100% 이상 급등", "급등 구간 평균 거래량이 직전 평균 대비 150% 이상 증가", "급등 고점봉 종가 대비 조정 구간 최저 종가 낙폭이 35% 이내", "조정 구간은 1~10주 사이", "조정 구간 종가는 급등 고점봉 종가를 넘지 않음"],
      "confirmation": ["주봉 MA10/30/40 표시", "조정 구간 변동성 축소", "조정 구간 종가 상단 제한 유지", "조정 구간 최저 종가가 급등 고점봉 종가 대비 35% 이내"],
      "invalidation": ["4~8주 안의 선행 급등률이 80% 미만", "급등 거래량이 직전 평균 대비 150% 미만", "조정 구간이 10주 초과", "조정 구간 종가가 급등 고점봉 종가를 초과", "조정 구간 최저 종가가 급등 고점봉 종가 대비 35% 초과 하락"],
